...
```bash
goto.svc$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
goto.svc$ sam local start-api
goto.svc$ curl http://localhost:3000/
```
...
```bash
goto.svc$ sam logs -n HelloWorldFunction --stack-name goto.svc --tail
```
...
```bash
goto.svc$ cd HelloWorldFunction
...
```bash
sam delete --stack-name goto.svc
```
