...
```bash
register.svc$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
register.svc$ sam local start-api
register.svc$ curl http://localhost:3000/
```
...
```bash
register.svc$ sam logs -n HelloWorldFunction --stack-name register.svc --tail
```
...
```bash
register.svc$ cd HelloWorldFunction
...
```bash
sam delete --stack-name register.svc
```
